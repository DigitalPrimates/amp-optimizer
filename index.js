...
console.log('Parsing ' + source + ' for ' + namespace + ' entries');
...
function parseFiles( fileList, namespace ) {
    ...
    function parseAST( contents, namespace ) {
        ...
        var dependencies =
            _(ast.body).
            filter( nodeType( 'VariableDeclaration' )).
        ...
        var declarations =
            ...
            filter( nodeType( 'FunctionExpression' )).
            flatMap( function( node ) {
            ...
            function(file) {
                return readFile(file, 'utf8').then(
                    function( contents ) {
                        ...
                        return Promise.resolve( {file:file, contents:contents, dependencies:parsed.dependencies, declarations:parsed.declarations} );
                    }
                )
            }
        )
...
function buildList( files ) {
    ...
    while( attempts<limit ) {
        ...
        var exported = _.keyBy(exportList, function(o) { return o.file });
...
function writeOutputFile( organizedFileList ) {
    fs.writeFile(output, _.map( organizedFileList, 'contents' ), function (err) {
        if (err) {
...
getContents( source ).
    ...
    then( buildList, logIt).
    then( writeOutputFile, logIt );
